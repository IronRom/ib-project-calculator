...
def _build_book_list(db, allow_regional: bool = True) -> str:
    ...
    lines = [
        ...
        "ПРАВИЛО ВЫБОРА БАЗЫ — ПО ИСТОЧНИКУ ФИНАНСИРОВАНИЯ (не по адресу!): "
        "справочники с пометкой [Регион: …] (московские МРР) применяй ТОЛЬКО если "
        "объект финансируется из БЮДЖЕТА ГОРОДА МОСКВЫ (городской заказ — заказчик "
        "Департамент/ГКУ/ГБУ города Москвы, АИП Москвы, средства бюджета города). "
        "Если объект финансируется из ФЕДЕРАЛЬНОГО бюджета (ФГБУ, ФКУ, ФГУП, "
        "министерство/ведомство, федеральная программа) или из ЧАСТНЫХ инвестиций — "
        "используй ТОЛЬКО федеральные справочники (СБЦ/НЗ), МРР НЕ применяй, даже "
        "если объект физически в Москве. Определи источник финансирования из ТЗ "
        "(заказчик, источник финансирования). НЕ создавай дублей по региональному и "
        "федеральному одновременно. ВАЖНО: правило выбирает БАЗУ, а НЕ отменяет вид "
        "работ: если ТЗ требует изыскания (ИГИ/ИГДИ/топосъёмка) — обязательно выбери "
        "справочник изысканий (МРР при городском заказе Москвы, иначе федеральный НЗ) "
        "и создай эти позиции.",
    ]
    for b in books:
...
═══ ВЫБОР НОРМАТИВНОЙ БАЗЫ — ПО ИСТОЧНИКУ ФИНАНСИРОВАНИЯ ═══

Сначала определи ИСТОЧНИК ФИНАНСИРОВАНИЯ объекта из ТЗ (заказчик, источник
финансирования, вид заказа) — это определяет всю нормативную базу:
  • ФЕДЕРАЛЬНЫЙ бюджет (заказчик ФГБУ/ФКУ/ФГУП, министерство/ведомство,
    федеральная целевая программа) ИЛИ ЧАСТНЫЕ инвестиции (коммерческий
    застройщик/инвестор, собственные средства) → ТОЛЬКО федеральные
    справочники: СБЦ (СБЦП) и НЗ. Московские МРР НЕ применять.
  • ГОРОДСКОЙ ЗАКАЗ МОСКВЫ (заказчик Департамент/ГКУ/ГБУ города Москвы,
    средства бюджета города Москвы, АИП города Москвы) → региональные
    справочники МРР (для видов работ, где они есть), приоритетнее федеральных.
Адрес объекта (в т.ч. «Москва») сам по себе базу НЕ определяет — определяет
именно ИСТОЧНИК ФИНАНСИРОВАНИЯ. Пример: ФГБУ в Москве = федеральный бюджет →
считается СБЦ/НЗ, а НЕ МРР.
...
ПОЛНОТА — ГЛАВНОЕ ТРЕБОВАНИЕ (не пропусти ни одной позиции):
  ...
  КАЖДУЮ применимую позицию ОТДЕЛЬНОЙ строкой:
    • ИЗЫСКАНИЯ (если в ТЗ есть слова «изыскания», «изыскательские», «предоставляется
      исполнителем») — каждый вид ОТДЕЛЬНОЙ позицией (category=new_construction), с
      соответствующим справочником изысканий (ИГДИ — геодезия, ИГИ — геология,
      ИГМИ/гидромет, ИЭИ — экология): инженерно-геодезические, инженерно-геологические,
    ...
    • ЗДАНИЯ и сооружения — по каждому ОДНА позиция по справочнику зданий
      (жилищно-гражданские/ОЖГС, промышленные и т.п.), X = площадь/объём.
    • СПЕЦИАЛЬНЫЕ СИСТЕМЫ, которые НЕ входят в норматив здания — каждая отдельно:
        слаботочные системы и ИТСО (видеонаблюдение СОТВ/СОТН, охранная и тревожная
        сигнализация СОТС, СКУД, связь, оповещение/эвакуация, пожарная сигнализация,
        серверы/АРМ ИТСО, СКС/ЛВС); автоматизация/диспетчеризация/АСУ/АСУТП.
    • НАРУЖНЫЕ СЕТИ (тепло-, водоснабжение, канализация, кабельные линии) — каждая своя.

  ВАЖНО — НЕ ДРОБИ ЗДАНИЕ: общие внутренние инженерные системы здания (отопление,
  вентиляция и кондиционирование, горячее и холодное водоснабжение, канализация,
  электроснабжение и электроосвещение, переустройство/ремонт помещений, усиление
  конструкций, фасады, кровля) УЖЕ учтены в нормативе цены самого здания (через доли
  разделов ПД/РД) — НЕ выделяй их отдельными позициями и НЕ создавай под них позиции с
  пустым справочником. Отдельно выделяются только изыскания, обследования, наружные
  сети, слаботочка/ИТСО и АСУ (см. выше).
  Не обрывай перечисление на первой крупной позиции. Пропущенная позиция — грубая
  ошибка сметы, но и лишнее дробление здания — тоже ошибка.
...
EXTRACTION_TOOL = {
    ...
    "input_schema": {
        ...
        "properties": {
            ...
            "stage": {
                "type": "string", "enum": ["П", "Р", "П+Р"],
                "description": "Стадии проектирования по ТЗ. «П+Р» — если требуется "
                               "И проектная, И рабочая документация (формулировки: «проектная и рабочая "
                               "документация», «ПД и РД», «стадии ПД, РД»). «П» — только если рабочая "
                               "документация явно не требуется. «Р» — только если проектная уже разработана "
                               "или прямо исключена. При сомнении — «П+Р».",
            },
            "region": {"type": "string"},
            "tz_object_name": {
                "type": "string",
