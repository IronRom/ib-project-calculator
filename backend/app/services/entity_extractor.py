...
Справочник выбирается по ВИДУ РАБОТ:
  ...
  Релейная защита (РЗА), распределительные
  устройства в промышленности                  → СБЦП 81-2001-13 (табл.11)
...
КОЭФФИЦИЕНТЫ — только те, что явно следуют из ТЗ:
  ...
  для ПИР (проектных работ) они не применяются.
...
Для позиций АСУТП/ПЛК/АРМ/SCADA указывай:
  ...
  x_value = null  (не используется)
  asutp_factors — объект с 7 факторами. Определи каждый из ТЗ:
