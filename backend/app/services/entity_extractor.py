...
НЕОЧЕВИДНЫЕ ПОЗИЦИИ:
  После извлечения явных объектов — рассуждай как опытный сметчик.
  ...
  Примеры нормативно-технологических связей (не исчерпывающий список):
  • КОС + сброс в рыбохозяйственный водоём (I, II кат.) →
      доочистка на фильтрах обязательна: ПДК по Росрыболовству без неё не достичь
  • КОС с биологической очисткой →
      образование осадка неизбежно → сооружения обработки осадка
  • КОС/НС + явное упоминание АСУ, АСДКУ, АСКП, диспетчеризации →
      коэффициент "asu" применяется ко всем позициям объекта
  • Напорный коллектор → камера переключения / выпуск на конце трассы
  • ВЗУ с несколькими скважинами → водовод от скважин до станции
